import { UserProfileDropdownProps } from "../../src/lib/locals/panelPage/declarations/interfacesCons";
...
export default function UserProfileDropdown({ user }: UserProfileDropdownProps): JSX.Element {
  const poRef = useRef<HTMLDivElement | null>(null),
    ...
    toggleContact = (): void => setContact(!shouldDisplayContact),
    [shouldDisplayPropDlg, setPropDlg] = useState<boolean>(false),
    togglePropDlg = (): void => setPropDlg(!shouldDisplayPropDlg),
    [shouldDisplayUserDlg, setUserDlg] = useState<boolean>(false),
    [, setUser] = useState<Readonly<User> | UserState | null>(user),
    [userClass, setClass] = useState<string>(user.loadedData.privilege),
    [userArea, setArea] = useState<string>(user.loadedData.area),
