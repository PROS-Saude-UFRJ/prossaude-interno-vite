    ...
    <ErrorBoundary FallbackComponent={() => <GenericErrorComponent message='Error rendering Table for Measures' />}>
      <div role='group' className='divTab' id='divTabMedAnt'>
        <table className='tabProgCons noInvert' id='tabMedAnt' itemScope>
          <caption className='tabLegProgCons' id='tabLegMedAnt' itemProp='headerMedAnt'>
            Medidas Antropométricas (exceto Dobras Cutâneas)
          </caption>
          <colgroup className='tabLegProgCons' id='tabColGrpMedAnt' itemProp='blockMedAnt'>
            {columns.map(nCol => (
              <Col ctx='MedAnt' nCol={nCol} key={`col_med-ant__${nCol}`} />
            ))}
          </colgroup>
          <thead className='tabTheadProgCons' id='tabTheadMedAnt' itemProp='blockMedAnt'>
            <tr className='tabRowProgCons tabRowMedAnt' id='tabRowMedAnt1' itemProp='rowMedAnt' data-row='1'>
              {columns.map(nCol => (
                <Th ctx='MedAnt' nCol={nCol} nRow={1} key={`th__med-ant__${nCol}`} />
              ))}
            </tr>
          </thead>
          <tbody className='tbodyProgCons' id='tbodyMedAnt' itemProp='blockMedAnt'>
            {rows.map(([nRow, lab], i) => (
              <tr
