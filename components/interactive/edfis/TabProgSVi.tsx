    ...
    <ErrorBoundary FallbackComponent={() => <GenericErrorComponent message='Error rendering Table for Vital Signs' />}>
      <div role='group' className='divTab' id='divTabSVi'>
        <table className='tabProgCons noInvert' id='tabProgSVi' itemScope>
          ...
          <colgroup className='tabColGrpProg' id='tabColGrpProgSVi' itemProp='blockSVi'>
            {columns.map(nCol => (
              <Col ctx='ProgSVi' nCol={nCol} key={`col_svi__${nCol}}`} />
            ))}
          </colgroup>
          <thead className='tabTheadProg' id='tabTheadProgSVi' itemProp='blockSVi'>
            <tr className='tabRowProg tabRowProgSVi' id='tabRowProgSVi1' itemProp='rowSVi'>
              {columns.map(nCol => (
